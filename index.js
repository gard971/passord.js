...
socket.on("loginReq", function(username, password, id){
    var encryptedPassword = MD5(password)
    if(username == "Gard" && encryptedPassword == "202cb962ac59075b964b07152d234b70"){
        var newKey = Math.random()
        approvedKeys.push(newKey)
        socket.emit("usernameCorrect", newKey, id)
         }
    else{
        socket.emit("usernameWrong")
    }
})
socket.on("check", function(key, requestId){
